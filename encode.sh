...
ffmpeg -i $IN \
    ...
    -codec:v libvpx -quality good -cpu-used 0 -b:v 1000k -qmin 10 -qmax 42 \
    -maxrate 1000k -bufsize 2000k -vf scale=$SCALE \
    -threads 0 \
    -an -pass 1 -f webm -y \
    /dev/null
...
ffmpeg -i $IN \
    -hide_banner -loglevel error -stats \
    -codec:v libx264 -profile:v main -preset slow -b:v 1000k \
    -maxrate 1000k -bufsize 2000k -vf scale=$SCALE \
    -threads 0 -pass 1 \
    -an -f mp4 -y \
    /dev/null
...
ffmpeg -i $IN \
    ...
    -maxrate 1000k -bufsize 2000k -vf scale=$SCALE \
    -threads 0 -pass 2 \
    -codec:a libfdk_aac -b:a 128k -f mp4 -y \
    $OUT.mp4
EOF
